...
export const getComponentColors = (p: AetherPalette) => {
  ...
  return {
    ...
    "editorWidget.resizeBorder": p.tabs.activeBorderTop,
    "editorSuggestWidget.focusHighlightForeground": p.editor.findMatch,
    "editorSuggestWidget.selectedForeground": p.list.activeSelectionFg,
    "editorSuggestWidget.selectedIconForeground": p.list.activeSelectionFg,
    "editorSuggestWidgetStatus.foreground": alpha(p.core.fg, Opacity.medium),

    "editorActionList.background": p.input.bg,
    "editorActionList.foreground": p.core.fg,
    "editorActionList.focusForeground": p.list.activeSelectionFg,
    "editorActionList.focusBackground": p.list.activeSelectionBg,
    "toolbar.activeBackground": alpha(p.core.fg, Opacity.low),

    "git.blame.editorDecorationForeground": alpha(p.core.fg, Opacity.medium),

    "scmGraph.foreground1": p.editor.brackets.pair1,
    "scmGraph.foreground2": p.editor.brackets.pair2,
    "scmGraph.foreground3": p.editor.brackets.pair3,
    "scmGraph.foreground4": p.editor.brackets.pair4,
    "scmGraph.foreground5": p.editor.brackets.pair5,
    "scmGraph.historyItemRefColor": p.status.info,
    "scmGraph.historyItemRemoteRefColor": p.syntax.functions,
    ...
    "terminalCommandDecoration.defaultBackground": alpha(
      p.core.fg,
      Opacity.medium
    ),
    "terminalCommandDecoration.successBackground": p.status.success,
    "terminalCommandDecoration.errorBackground": p.status.error,
    "terminalCommandGuide.foreground": alpha(p.core.fg, Opacity.low),
    "terminal.hoverHighlightBackground": alpha(p.editor.findMatch, Opacity.low),
    "terminal.dropBackground": alpha(p.editor.selection, Opacity.subtle),
    "terminal.findMatchBorder": p.editor.findMatch,
    "terminal.findMatchHighlightBorder": alpha(p.editor.findMatch, Opacity.medium),
    "terminal.initialHintForeground": alpha(p.core.fg, Opacity.medium),

    "terminalStickyScroll.background": p.panel.bg,
    "terminalStickyScroll.border": p.panel.border,
    "terminalStickyScrollHover.background": p.list.hoverBg,
    "terminalOverviewRuler.cursorForeground": p.editor.cursor,
    "terminalOverviewRuler.findMatchForeground": p.editor.findMatch,
    "terminalOverviewRuler.border": p.panel.border,

    "terminalSymbolIcon.aliasForeground": p.syntax.functions,
    "terminalSymbolIcon.argumentForeground": p.syntax.variables,
    "terminalSymbolIcon.branchForeground": p.git.modified,
    "terminalSymbolIcon.commitForeground": p.syntax.constants,
    "terminalSymbolIcon.fileForeground": p.core.fg,
    "terminalSymbolIcon.flagForeground": p.syntax.keywords,
    "terminalSymbolIcon.folderForeground": p.syntax.types,
    "terminalSymbolIcon.inlineSuggestionForeground": alpha(
      p.core.fg,
      Opacity.medium
    ),
    "terminalSymbolIcon.methodForeground": p.syntax.functions,
    "terminalSymbolIcon.optionForeground": p.syntax.keywords,
    "terminalSymbolIcon.optionValueForeground": p.syntax.numbers,
    "terminalSymbolIcon.pullRequestForeground": p.syntax.functions,
    "terminalSymbolIcon.pullRequestDoneForeground": p.status.success,
    "terminalSymbolIcon.remoteForeground": p.status.info,
    "terminalSymbolIcon.stashForeground": p.syntax.classes,
    "terminalSymbolIcon.symbolicLinkFileForeground": p.status.info,
    "terminalSymbolIcon.symbolicLinkFolderForeground": p.status.info,
    "terminalSymbolIcon.tagForeground": p.syntax.tags,
    "terminalSymbolIcon.symbolText": p.core.fg,

    "chat.avatarBackground": p.input.bg,
    "chat.avatarForeground": p.core.fg,
    "chat.editedFileForeground": p.git.modified,
    "chat.linesAddedForeground": p.git.added,
    "chat.linesRemovedForeground": p.git.deleted,
    "chat.requestCodeBorder": p.core.border,
    "chat.requestBubbleBackground": alpha(p.input.bg, Opacity.half),
    ...
    "inlineChatInput.background": p.input.bg,
    "inlineChatInput.border": p.input.border,
    "inlineChatInput.focusBorder": p.input.focusBorder,
    "inlineChatInput.placeholderForeground": p.input.placeholder,
    "inlineChatDiff.inserted": alpha(p.git.added, Opacity.subtle),
    "inlineChatDiff.removed": alpha(p.git.deleted, Opacity.subtle),
    "interactive.activeCodeBorder": p.tabs.activeBorderTop,
    "interactive.inactiveCodeBorder": p.core.border,

    "debugToolBar.border": p.core.border,
    "debugView.exceptionLabelForeground": p.core.fg,
    "debugView.exceptionLabelBackground": alpha(p.status.error, Opacity.half),
    "debugView.stateLabelForeground": p.core.fg,
    "debugView.stateLabelBackground": alpha(p.core.fg, Opacity.subtle),
    "debugView.valueChangedHighlight": p.status.info,
    "debugTokenExpression.type": p.syntax.types,

    "testing.coveredBackground": alpha(p.status.success, Opacity.faint),
    "testing.coveredBorder": alpha(p.status.success, Opacity.medium),
    "testing.coveredGutterBackground": alpha(p.status.success, Opacity.low),
    "testing.uncoveredBackground": alpha(p.status.error, Opacity.faint),
    "testing.uncoveredBorder": alpha(p.status.error, Opacity.medium),
    "testing.uncoveredGutterBackground": alpha(p.status.error, Opacity.low),
    "testing.uncoveredBranchBackground": alpha(p.status.warning, Opacity.low),
    "testing.coverCountBadgeBackground": p.button.primary.bg,
    "testing.coverCountBadgeForeground": p.button.primary.fg,
    "testing.message.info.decorationForeground": p.status.info,
    "testing.message.info.lineBackground": alpha(p.status.info, Opacity.subtle),
  };
};
